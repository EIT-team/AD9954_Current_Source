...
#include <SPI.h> // for AD5944

#define CLOCK 20e6 // this is set by the resistor on the board 
...
#include "PCBPinsRev2.h" // Pins for PCB version - these have been altered to more logical layout for PCB
...
const int NumShunt = 12; // number of SINKS to shunt together

int Injection[maxInjections][NumShunt+1] = { 0 }; //number of injections in protocol - max 200 to avoid dynamic memory allocation
...
void dostuff()
{
	...
	case 1: // start injection state
	{
		...
		if (PC_inputgoodness && 1) // only do anything if settings are ok
		{
			//CS_Disp(MSG_Start);
			...
			else // we are in multifrequency mode and thus we need to set more stuff before we start injection
			{
				...
				boolean AutoOffOK = 1; //set ranging to off
				boolean RangeSetOK = 1; // set range to max required
		...
		*/
		...
		if (SingleFreqMode) // if only doing 1 frequency then we can copy and paste the code from previous iterations
		{
			if (!SwitchesProgrammed)
			{
				...
				SetSwitchesFixed_shunt(); // if switches havent been programmed then do that based on iPrt and take a set amount of time
			}
