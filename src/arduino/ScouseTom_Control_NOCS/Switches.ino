...
//function to program switches so they are shunted
void programswitches_shunt(int injchn[], int maxpins)
{
/*
	for (int l=0; l < NumShunt+1;l++)
	{
		Serial.print(injchn[l]);
		Serial.print(",");
	}
	Serial.println("");
*/
  //Define which are sources and which are sinks
  int sourcechn= injchn[0];
  //Serial.print("Source ");
  //Serial.println(sourcechn);
  ...
  for (int j = maxpins; j > 0; j--) {
    ...
    for (int i =0; i< NumShunt; i++)
    {
      
      if (injchn[i+1] == j)  
	  {
		  //Serial.println(j);
		  digitalWrite(DINn, HIGH);
	  }
